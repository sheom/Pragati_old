...
export const getNewMISData = async (req, res) => {
  ...
  if (year > 2024) {
    ...
    let budgetMISData = await getBudgetData(propertyCode, year);
    let budgetMISData_last = await getBudgetData(propertyCode, year-1);

    let actualMISData = await getActualData(propertyCode, year - 1);
    let actualMISData_last = await getActualData(propertyCode, year - 2);
    ...
    if (propertyCode === "PHL") {
      ...
      processBudgetData(budgetMISData_last.data[0]);
      processActualData(actualMISData_last.data);
      ...
      processBudgetData(budgetMISData.data[0]);
      //
      //console.log("returnTemplate.data.int_rev.actual from Push: "+returnTemplate.data.int_rev.actual)
      //console.log("returnTemplate.data.int_rev: "+JSON.stringify(returnTemplate.data.int_rev))
...
const processActualData = (data) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    //
    //console.log("data["+i+"].payload.inputIntRevenue: "+data[i].payload.inputIntRevenue);
    intRevData.actual[index] = data[i].payload.inputIntRevenue;
    //console.log("intRevData.actual["+index+"]: "+intRevData.actual[index]);
  ...
  //
  //
  console.log("intRevData.actual: "+ intRevData.actual);
  returnTemplate.data.int_rev.actual = [...intRevData.actual];
  console.log("returnTemplate.data.int_rev.actual: "+returnTemplate.data.int_rev.actual)
  //
